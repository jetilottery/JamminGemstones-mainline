define(function module(require){
	...
	function ResultPanelController(){
		...
		this.winAnimationEmitters = [];
		this.winAnimationEmitters.length = 4;
		this.celeParticles = ['winAnimBlue','winAnimGreen','winAnimOrange','winAnimYellow'];
		...
		this.suppressNonWinResultPlaque = true;
	...
	ResultPanelController.prototype.init = function(){
		...
		this.celeContainer = gr.lib._celeParticles.pixiContainer;
		...
		this.winClickArea = gr.lib._winPlaqueClickArea;
		this.noWinClickArea = gr.lib._noWinPlaqueClickArea;
	...
	ResultPanelController.prototype.onBeginNewGame = function(){
		this.onEndHide = false;
	};

	ResultPanelController.prototype.changeBackgroundBGIfPortrait = function(data){
		this.isPortrait = data;
	};
	...
	/* 	this.celeEmitter = new Particle(
			this.celeContainer, 
			'',
			this.celeParticles, 
			ResultPanelController.gemParticleProperties,
			false
		); */
	...
	// 	this.emitter_coinRain = new Particles.Emitter(this.rainContainer, [
	// 		{ //coin
	// 			framerate:12,
	// 			loop:true,
	...
	// 	}
	// };
	ResultPanelController.prototype.startCelebration = function (){
		var gemParticleProperties;
		if(Math.floor(this.resultData.prizeValue/this.currentPricePoint) > 20){
			if(this.isPortrait){
				gemParticleProperties = ResultPanelController.gemMoreParticleProperties_portrait;
			}else{
				gemParticleProperties = ResultPanelController.gemMoreParticleProperties;	
			}
		}else{
			if(this.isPortrait){
				gemParticleProperties = ResultPanelController.gemFewParticleProperties_portrait;
			}else{
				gemParticleProperties = ResultPanelController.gemFewParticleProperties;	
			}
		}
		for(var i = 0; i < this.winAnimationEmitters.length; i++){
			this.winAnimationEmitters[i] = new Particle(
				this.celeContainer, 
				this.celeParticles[i],
				[],
				gemParticleProperties,
				true,
				{startFrame: 1, endFrame: 16, isSingleNumFormat: false, frameRate: 60}
			);
			this.winAnimationEmitters[i].start();
			this.winAnimationEmitters[i].isRuning = true;
		}
	};
	ResultPanelController.prototype.stopCelebration = function (){
		for(var i = 0; i < this.winAnimationEmitters.length; i++){
			if(this.winAnimationEmitters[i] && this.winAnimationEmitters[i].isRuning){
				this.winAnimationEmitters[i].isRunning = false;
				this.winAnimationEmitters[i].stop();
			}
		}
	};
...
/* 		if(this.emitter_star.isRunning === true){
		...
		if(this.celeEmitter.isRunning === true){
			this.celeEmitter.update(deltaTime * 0.05);
		}
	...
	ResultPanelController.prototype.onGameParametersUpdated = function () {
		...
		gr.lib._noWinText.autoFontFitText = true;
		gr.lib._noWinText.setText(loader.i18n.Game.message_nonWin);
		...
		if (SKBeInstant.config.wagerType === 'TRY')
		{
			if(Number(SKBeInstant.config.demosB4Move2MoneyButton) === -1) {
				this.totalWinTryText.setText(loader.i18n.Game.message_tryWin);
		...
		this.totalWinBuyText.setText(loader.i18n.Game.message_buyWin);
	...
	ResultPanelController.prototype.hideDialog = function() {
		if(this.winPanelSprite){
			this.winPanelSprite.show(false);
		}
		gr.lib._nonWinPlaque.show(false);
		this.stopCelebration();
	};
	ResultPanelController.prototype.winReaction = function(){
		...
		if(Math.floor(this.resultData.prizeValue/this.currentPricePoint) >= 5){
			this.startCelebration();
		}
		this.winPanelSprite.show(true);
	};
	
	ResultPanelController.prototype.offCloseWinResultPanel = function() {
		var _this = this;
		this.winClickArea.off('click', function(){ _this.closeWinResultPanel();});
		this.winClickArea.off('mousedown', function(){ _this.closeWinResultPanel();});
		this.winClickArea.pixiContainer.$sprite.interactive = false;
		this.winClickArea.pixiContainer.$sprite.cursor = "default";
		this.hideDialog();
	};
	
	ResultPanelController.prototype.onCloseWinResultPanel = function() {
		var _this = this;
		this.winClickArea.pixiContainer.$sprite.interactive = true;
		this.winClickArea.pixiContainer.$sprite.cursor = "pointer";
		this.winClickArea.on('click',function(){_this.offCloseWinResultPanel();});
		this.winClickArea.on('mousedown',function(){_this.offCloseWinResultPanel();});
	};
	
	ResultPanelController.prototype.offCloseNoWinResultPanel = function() {
		var _this = this;
		this.noWinClickArea.off('click', function(){ _this.offCloseNoWinResultPanel();});
		this.noWinClickArea.off('mousedown', function(){ _this.offCloseNoWinResultPanel();});
		this.noWinClickArea.pixiContainer.$sprite.interactive = false;
		this.noWinClickArea.pixiContainer.$sprite.cursor = "default";
		this.hideDialog();
	};
	
	ResultPanelController.prototype.onCloseNoWinResultPanel = function() {
		var _this = this;
		this.noWinClickArea.pixiContainer.$sprite.interactive = true;
		this.noWinClickArea.pixiContainer.$sprite.cursor = "pointer";
		this.noWinClickArea.on('click',function(){_this.offCloseNoWinResultPanel();});
		this.noWinClickArea.on('mousedown',function(){_this.offCloseNoWinResultPanel();});
	};
	
	ResultPanelController.prototype.showDialog = function() {
		if(!this.isShowPanelBG()){return;}
		if(this.isShowNoWinResultPlaque()){
			if(this.resultData.playResult === 'WIN'){
				this.onCloseWinResultPanel();
				this.winReaction();
			}else{
	...
	ResultPanelController.prototype.isShowNoWinResultPlaque = function (){
		let rs = true;
		if(SKBeInstant.config.customBehavior){
	...
	ResultPanelController.prototype.onStartUserInteraction = function(data) {
		if (data.price && SKBeInstant.config.gameType === 'ticketReady') {
            this.currentPricePoint = data.price;
        } 
	...
	ResultPanelController.prototype.onPlayerWantsPlayAgain = function (prizePoint){
		this.currentPricePoint = prizePoint;
		if(!this.onEndHide){
			this.onEndHide = true;
			this.hideDialog();
		}
	...
	ResultPanelController.gemMoreParticleProperties = {
		"alpha": {
			"start": 1,
			"end": 1
		},
	...
	ResultPanelController.gemFewParticleProperties = {
		"alpha": {
			"start": 1,
			"end": 1
		},
	...
	ResultPanelController.gemMoreParticleProperties_portrait = {
		"alpha": {
			"start": 1,
			"end": 1
		},
	...
	ResultPanelController.gemFewParticleProperties_portrait = {
		"alpha": {
			"start": 1,
			"end": 1
		},
