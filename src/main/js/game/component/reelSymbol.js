...
define(function module(require) {
	...
	function ReelSymbol(data) {
		...
		this.mainSprite.pixiContainer.removeChildren(); // empty the children node, make it an empty container.
	...
	ReelSymbol.prototype.createHoneyCombDisplay = function () {
		this.honeycombDisplay = new Sprite({
			...
			"_style": {
				...
				"_top": 6,
				"_left":4,*/
				"_top": this.getOriginalPos().y + 28,
				"_left": this.getOriginalPos().x + 28,
				"_background": {
	...
	*/
	...
	ReelSymbol.prototype.applySymbolCode = function (code) {
		...
		if (img) {
			...
			this.planetDisplay.setImage(img + "Anim_0000");
		}
		else {
			throw new Error("Can not find image for " + code);
		}
	};
	ReelSymbol.prototype.applyBonusCode = function (code) {
		this.currentAnimName = "";
		this.currentBonusCode = code;
		//console.log(startFrameNumber);
		switch (code) {
			case '1':
				this.currentAnimName = 'bonus1';
				this.ufoFireEvent = LittleGreenMenGameEvent.eventIDs.UFONAVY_SPACESHIP_FIRE;
				this.showBonusDisplay();
				this.bonusDisplay.setImage(this.currentAnimName);
				this.scalePlanetDisplay(0.9); 
				//this.bonusDisplay.gotoAndPlay(this.currentAnimName, 0.2, true, startFrameNumber);
				break;
			case '2':
				this.currentAnimName = 'bonus2';
	...
	ReelSymbol.prototype.scalePlanetDisplay = function (val) {
		this.planetDisplay.updateCurrentStyle({ _transform: { _scalePlanetDisplay: { _x: val, _y: val } } });
	};
	ReelSymbol.prototype.rotatePlanet = function (val) {
		this.planetDisplay.updateCurrentStyle({ "_transform": { "_rotate": val } });
	};
	ReelSymbol.prototype.scalePlanet = function (val) {
	...
	ReelSymbol.prototype.highLight = function () {
		this.showHoneyComb();
		this.scalePlanetDisplay(1.1);
		const _this = this;
		gr.getTimer().setTimeout(function () {
			_this.scalePlanetDisplay(1);
			_this.hideHoneyComb();
			_this.playWinningAnimation();
		}, config.timers.baseGame_symbolHoneyCombLastTimer);
	};
	ReelSymbol.prototype.playWinningAnimation = function (isInWrapBonus = false) {
		//this.hide();
		const _this = this;
		/*
		...
		*/
		if(this.getSymbolCode() === 'IW'){
			gr.lib._IWSpine.updateCurrentStyle({"_top": this.orgPosition.y});  
			gr.lib._IWSpine.spine.state.setAnimation(0, 'instantWinAnim_Intro', false).listener = {
				complete: function () {
					_this.onWinningAnimationComplete(isInWrapBonus);
					gr.lib._IWSpine.spine.state.setAnimation(0, 'instantWinAnim_Loop', true);
				}
			};

			gr.lib._IWwinValue.updateCurrentStyle({"_top": this.orgPosition.y}); 
			gr.lib._IWwinValue.show(true);

		}else{
			const animName = this.symbolsMap[this.currentSymbolCode]+"Anim";
			this.planetDisplay.onComplete = function(){
				_this.onWinningAnimationComplete(isInWrapBonus);
			};
			this.planetDisplay.gotoAndPlay(animName, 0.8);
		}
		...
		if(isInWrapBonus){
			msgBus.publish(LittleGreenMenGameEvent.eventIDs.SINGLE_SYMBOL_WIN_EXPLOSION_COMPLETE, this);
		}
	};
	...
	// };
	ReelSymbol.prototype.onWinningAnimationComplete = function (isInWrapBonus) {
		this.hide();
		
		if(isInWrapBonus !== true){
			msgBus.publish(LittleGreenMenGameEvent.eventIDs.SINGLE_SYMBOL_WIN_EXPLOSION_COMPLETE, this);
		}
	};
