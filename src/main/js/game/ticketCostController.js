 define(function module(require) {
     ...
     function changeBackgroundBGIfPortrait(data) {
         isPortrait = data;
         if (boughtTicket) {
             if (isPortrait) {
                 if (!(isPhase1())) {
     ...
     function onGameParametersUpdated() {
         ...
         var style = {
             ...
             "_style": {
                 ...
                 "_left": "196",
                 "_background": {
                     "_imagePlate": "ticketCostLevelOff"
                 },
                 "_top": "128",
                 "_transform": {
                     "_scale": {
         ...
         if (prizePointList.length <= 1) {
             ...
             gr.lib._dimTicketCost.show(false);
         } else {
         ...
         gr.lib._ticketCost.show(isSKBInGIP() === false && isShowTutorialAtBeginning() === false && !(isPhase1()));
     }
     ...
     function setTicketCostValue(prizePoint) {
         ...
         if (ticketIconObj) {
             ticketIconObj.setImage('ticketCostLevelOff');
         }
         ticketIconObj = gr.lib[ticketIcon[prizePoint]];
         ticketIconObj.setImage('ticketCostLevelOffOn');
         if (_currentPrizePoint !== prizePoint) {
     ...
     function decreaseButtonAlpha(alphaNum) {
         plusButton.setAlpha(alphaNum);
         minusButton.setAlpha(alphaNum);
     }

     function increaseButtonAlpha() {
         plusButton.setAlpha(1);
         minusButton.setAlpha(1);
     }
     ...
     msgBus.subscribe('changeBackgroundBGIfPortrait', changeBackgroundBGIfPortrait);
     ...
     msgBus.subscribe(LittleGreenMenGameEvent.eventIDs.STANDARD_BONUS_ENTRE_TRANSITION_START, startSBonusStart);
     msgBus.subscribe(LittleGreenMenGameEvent.eventIDs.STANDARD_BONUS_EXIT_TRANSITION_COMPLETE, startSBonusComplete);
     return {};
 });