 ...
 * @Last Modified time:	2020-05-13 12:21:54
 */
define(function module(require) {
    ...
    ControlPanel.prototype.initButtonLabel = function() {
        ...
        // Fix for JAMGEM-188: TUR- Buy button text is getting very small.
    ...
    ControlPanel.prototype.changeBackgroundBGIfPortrait = function() {
        if (this.bln_gameStarted) { this.decreaseButtonAlpha(0.4); }
    };
        ...
        if (this.isSKBInGIP()) {
            ...
            if (this.isShowTutorialAtBeginning() === true) {
                ...
                if (this.isAudioDialogEnabled() === false) {
                    //this.playBtnOnClick();
                    this.showPlayButton();
                    this.infoBtn.enable(true);
                    this.infoBtn.setAlpha(1);
                }
            }
        } else {
    ...
    ControlPanel.prototype.onTutorialIsHide = function() {
        ...
        if (this.isPlayResponseReceived()) {
            ...
            if (this.bln_playAgainIsOn) {
                if (this.isGameOneTimeOnly() === true) {
                    this.infoBtn.show(true);
                    this.infoBtn.enable(true);
                    this.infoBtn.setAlpha(1);
                    this.exitBtn.show(true);
                    this.exitBtn.enable(true);
    ...
    ControlPanel.prototype.showPlayButton = function() {
        if (this.isEveryThingOkay()) {
            ...
            this.infoBtn.show(true);
            this.infoBtn.enable(false);
            this.infoBtn.setAlpha(0.4);
            if (this.isHomeButtonAllowed()) {
                this.homeBtn.show(true);
                this.homeBtn.enable(false);
                this.homeBtn.setAlpha(0.4);
    ...
    */
    ...
    ControlPanel.prototype.isHomeButtonAllowed = function() {
        ...
        //return SKBeInstant.isSKB() === true? false: true;
    };
    ControlPanel.prototype.buyBtnOnClick = function() {
        gr.lib._buttonAnim.gotoAndStop(0);
        gr.lib._buttonAnim.show(false);
        msgBus.publish('disableUI');
        this.playSoundByConfig('ButtonBuy');
        this.disableAllButton();
        this.decreaseButtonAlpha(0.4);
    ...
    ControlPanel.prototype.tryBtnOnClick = function() {
        this.buyBtnOnClick();
        gr.lib._tryButtonAnim.gotoAndStop(0);
        gr.lib._tryButtonAnim.show(false);
    };
    ...
    ControlPanel.prototype.showAllButton = function() {
        if (this.isSKBInGIP() === false && this.isGameStarted() === false && this.bln_tutorialIsOn == false && this.isEveryThingOkay()) {
            if (this.isBuyMode()) {
                ...
                gr.lib._buttonAnim.show(true);
                gr.lib._buttonAnim.gotoAndPlay("buttonAnim", 0.3, true);
                this.buyBtn.enable(true);
                this.tryBtn.show(false);
                this.tryBtn.enable(true);
                this.MTMBtn.show(false);
                this.MTMBtn.enable(true);
            } else {
                this.tryBtn.show(true);
                this.buyBtn.enable(true);
                this.buyBtn.show(false);
                this.tryBtn.enable(true);
                gr.lib._tryButtonAnim.show(true);
                gr.lib._tryButtonAnim.gotoAndPlay("buttonAnim", 0.3, true);
                if (this.isNoMTMBtnMode() === false) {
                    this.MTMBtn.show(true);
                    this.MTMBtn.enable(true);
                    this.tryBtn.show(true);
                    ...
                    this.tryBtn.show(false);
                    this.buyButtonText.setText(loader.i18n.Game.button_try);
                    this.buyBtn.show(true);
                    // center the try button by apply buy button's position _left;
                    //this.tryBtn.sprite.updateCurrentStyle({"_left":this.buyBtn.sprite.data._style._left || 0});
                }
            }
            this.infoBtn.show(true);
            this.infoBtn.enable(true);
            this.homeBtn.show(this.isHomeButtonAllowed());
            this.homeBtn.enable(true);
        }
        this.increaseButtonAlpha();
    };
    ...
    ControlPanel.prototype.disableAllButton = function() {
        this.homeBtn.enable(false);
        this.infoBtn.enable(false);
        ...
        //this.autoPlayBtn.show(false);
        // this.playAgainBtn.show(false);
        this.exitBtn.enable(false);
        //this.playBtn.show(false);

        //try section
        this.tryBtn.enable(false);
        this.MTMBtn.enable(false);
    };
    ControlPanel.prototype.decreaseButtonAlpha = function(alphaNum) {
        this.homeBtn.setAlpha(alphaNum);
        this.infoBtn.setAlpha(alphaNum);

        this.buyBtn.setAlpha(alphaNum);
        //this.autoPlayBtn.show(false);
        // this.playAgainBtn.show(false);
        this.exitBtn.setAlpha(alphaNum);
        //this.playBtn.show(false);

        //try section
        this.tryBtn.setAlpha(alphaNum);
        this.MTMBtn.setAlpha(alphaNum);
    };

    ControlPanel.prototype.increaseButtonAlpha = function() {
        ...
        //try section
        this.tryBtn.setAlpha(1);
        this.MTMBtn.setAlpha(1);
    };
