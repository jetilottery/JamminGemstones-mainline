...
define(function module(require){
    ...
    TutorialController.prototype.handleL2P= function(portrait){
        isPortrait = portrait;
        for (let i = minIndex; i <= maxIndex; i++) {
            let obj = gr.lib['_tutorialPage_0'+i+'_Text_00'];
            let line;
    ...
    TutorialController.prototype.setupButtons = function (){
        ...
        this.buttonClose = new GladButton(gr.lib._buttonCloseTutorial, gr.lib._buttonCloseTutorial.getCurrentStyle()._background._imagePlate, scaleType);
    ...
    */
    ...
    TutorialController.prototype.setupText = function (){
        ...
        for (var i = minIndex; i <= maxIndex; i++) {
            ...
            obj.originalFontSize = obj._currentStyle._font._size;
    ...
    TutorialController.prototype.handleTutorialContent = function (parentSpr, linesArr) {
        ...
        function setColor(sText){
            if (!(curStyle._text._gradient._color.length===2&&curStyle._text._gradient._color[1]===curStyle._text._gradient._color[0])) {
                var colorArr=[];
                for (var i = 0; i < curStyle._text._gradient._color.length; i++) {
                        colorArr[i]="#"+curStyle._text._gradient._color[i];
                }
                sText.style.fill=colorArr;
                sText.style.fillGradientStops=curStyle._text._gradient._stop;
                if (curStyle._text._gradient._orientation==="horizontal") {
                    sText.style.fillGradientType=PIXI.TEXT_GRADIENT.LINEAR_HORIZONTAL;
                }else{
                    sText.style.fillGradientType=PIXI.TEXT_GRADIENT.LINEAR_VERTICAL;
                }
            }
        }
        ...
        function setPosition() {
            ...
            for (var i = 0; i < parentSpr.pixiContainer.children.length; i++) {
                ...
                if (i === 0) {
                    ...
                    if(prevLine.children.length > 0){ //contain image
                        if(isPortrait){
                            line.y = prevLine.y + prevLine.height -25;
                        }else{
                            line.y = prevLine.y + prevLine.height -15;
                        }
                    }else{
                        if(isPortrait){
