...
/*     function playBGAnim() {
        var speed = [0.2,0.3,0.4];
        for (let i = 1; i < 5; i++) {
            gr.lib['_floating'+i].gotoAndPlay('floatingAnim',speed[Math.random()*speed.length],true);
        }
    } */
    ...
    function updateBG(){
        ...
        if(!gr.lib._BGpanel.spine){
            spineSymbol = new PIXI.spine.Spine(resLib.spine.backGroundAnim.spineData);
			style = {x:720, y:405,scaleX:1, scaleY:1, alpha:1};
			gameUtils.setSpineStype(spineSymbol, style);
			gr.lib._BGpanel.pixiContainer.addChild(spineSymbol);
			gr.lib._BGpanel.spine = spineSymbol;
			spineSymbol.state.setAnimationByName(0, 'baseBG', true);
        }
        if(!gr.lib._bonusBGpanel.spine){
            spineSymbol = new PIXI.spine.Spine(resLib.spine.backGroundAnim.spineData);
			style = {x:720, y:405,scaleX:1, scaleY:1, alpha:1};
			gameUtils.setSpineStype(spineSymbol, style);
			gr.lib._bonusBGpanel.pixiContainer.addChild(spineSymbol);
			gr.lib._bonusBGpanel.spine = spineSymbol;
			spineSymbol.state.setAnimationByName(0, 'bonusBG', true);
        }
        if(!gr.lib._BGpanel_P.spine){
            spineSymbol = new PIXI.spine.Spine(resLib.spine.backGroundAnim.spineData);
			style = {x:405, y:720,scaleX:1, scaleY:1, alpha:1};
			gameUtils.setSpineStype(spineSymbol, style);
			gr.lib._BGpanel_P.pixiContainer.addChild(spineSymbol);
			gr.lib._BGpanel_P.spine = spineSymbol;
			spineSymbol.state.setAnimationByName(0, 'baseBG_Portrait', true);
        }
        if(!gr.lib._bonusBGpanel_P.spine){
            spineSymbol = new PIXI.spine.Spine(resLib.spine.backGroundAnim.spineData);
			style = {x:405, y:720,scaleX:1, scaleY:1, alpha:1};
			gameUtils.setSpineStype(spineSymbol, style);
			gr.lib._bonusBGpanel_P.pixiContainer.addChild(spineSymbol);
			gr.lib._bonusBGpanel_P.spine = spineSymbol;
			spineSymbol.state.setAnimationByName(0, 'bonusBG_Portrait', true);
        }
    }
