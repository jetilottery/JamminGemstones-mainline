...
//    }
	...
	function onSplashLoadDone(){
        ...
        var logoName;
        if(config.predefinedStyle.splashLogoName){
            logoName = config.predefinedStyle.splashLogoName;
        }else if(config.predefinedStyle[orientation].splashLogoName){
            logoName = config.predefinedStyle[orientation].splashLogoName;
        }

		if(spriteSheetAnimationMap[logoName]){
			logoSprite=PIXI.extras.AnimatedSprite.fromFrames(spriteSheetAnimationMap[logoName]);
			logoSprite.animName=logoName;
		}else{
			logoSprite = new PIXI.Sprite(PIXI.Texture.fromFrame(logoName));
		}
		applyLogoStyle(pdd);
