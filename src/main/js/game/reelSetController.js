...
define(function module(require) {
	...
	function ReelSetController() {
		...
		this.removeSymol = true;
		this.symbolAnimationCompleteCallback = new CallbackFunc(this, this.singleSymbolWinAnimComplete);
		this.textRollingCompleteCallback = new CallbackFunc(this, this.goNext);
		this.bonusGameInProgress = false;
		this.addListeners();
		window.reelset = this;
	}
	ReelSetController.prototype.addListeners = function () {
		...
		msgBus.subscribe('ticketCostChanged', new CallbackFunc(this, this.moveSymbolsOffInstantly));
		msgBus.subscribe('jLottery.beginNewGame', new CallbackFunc(this, this.onBeginNewGame));
		msgBus.subscribe(LittleGreenMenGameEvent.eventIDs.SINGLE_SYMBOL_WIN_EXPLOSION_COMPLETE, this.symbolAnimationCompleteCallback);
		...
		msgBus.subscribe(LittleGreenMenGameEvent.eventIDs.ACTION_BONUS_ENTRE_TRANSITION_COMPLETE, new CallbackFunc(this, this.takeAnAction));		
		msgBus.subscribe(LittleGreenMenGameEvent.eventIDs.BASEGAME_ET_FADEOUTCOMPLETED, new CallbackFunc(this, this.startGame));
		msgBus.subscribe(LittleGreenMenGameEvent.eventIDs.PAYTABLE_TEXT_ROLL_UP_COMPLETE, this.textRollingCompleteCallback);
		...
		// To determine if we are in the bonus game or not
		msgBus.subscribe(LittleGreenMenGameEvent.eventIDs.STANDARD_BONUS_ENTRE_TRANSITION_COMPLETE, new CallbackFunc(this, this.onBonusEntry));
		msgBus.subscribe(LittleGreenMenGameEvent.eventIDs.STANDARD_BONUS_EXIT_TRANSITION_COMPLETE, new CallbackFunc(this, this.onBonusExit));
		msgBus.subscribe(LittleGreenMenGameEvent.eventIDs.FORCE_NEXT_TURN, new CallbackFunc(this, this.handleForceNextTurn));
	};

	ReelSetController.prototype.onBonusEntry = function(){
		this.bonusGameInProgress = true;
	};

	ReelSetController.prototype.onBonusExit = function(){
		this.bonusGameInProgress = false;
	};

	// Force the next turn if we are in the base game
	ReelSetController.prototype.handleForceNextTurn = function(){
		if (!this.bonusGameInProgress) this.goNext();
	};

	ReelSetController.prototype.onReInitialize = function(){
		this.moveSymbolsOffInstantly();
	};

	ReelSetController.prototype.onBeginNewGame  = function(){
		this.removeSymol = false;
	};
	/*
	...
	*/
	ReelSetController.prototype.gameParametersUpdated = function () {
		...
		this.spaceshipCommander = new UFOController({
			numOfLights: 4,
			idPrefix: '_spaceship',
			collectionSound: 'collection',
			mainAnimName: 'spaceship',
			lightAnimName: 'selectionBonusGolden',
			spineStyle:{x:56, y:56,scaleX:1, scaleY:1, alpha:1},
			static:"bonus1",
			mainAnimSpeed: 0.1,
			carrierHomePosOffset: { x: 30, y: 23 },
		...
		this.meteoriteCommander = new UFOController({
			...
			lightAnimName:'selectionBonusSilver',
			spineStyle:{x:56, y:56,scaleX:1, scaleY:1, alpha:1},
			static:"bonus2",
			mainAnimSpeed: 0.1,
			carrierHomePosOffset: { x: 33, y: 23 },
	...
	ReelSetController.prototype.newMask = function(style){		
		let mask = new PIXI.Graphics();
		mask._name = "reelsetMask";
		...
		for (let i = 0; i <= 6; i++) {
			...
			if (i == 0) {
				mask.moveTo(roundPointLeftX, roundPointLeftY);
			}
			else {
				mask.lineTo(roundPointLeftX, roundPointLeftY);
			}
			mask.quadraticCurveTo(controlPointX, controlPointY, roundPointRightX, roundPointRightY);
	...
	ReelSetController.prototype.handleL2P = function(isPortrait){
		this.mask = isPortrait? this.maskP: this.maskL;
		gr.lib._Planet.pixiContainer.mask = this.mask;
	};
	ReelSetController.prototype.createMask = function () {
		this.maskL = this.newMask(config.positions.baseGameReelsetDrawMask.landscape);
		this.maskP = this.newMask(config.positions.baseGameReelsetDrawMask.portrait);
		this.mask =  this.isPortrait()? this.maskP: this.maskL;
		gr.lib._Planet.pixiContainer.mask = this.mask;
	};
	...
	ReelSetController.prototype.startGame = function () {
	...
	ReelSetController.prototype.parseResponseData = function (data) {
		...
		//const scenarioData = "C0C0F0F0F0E0D0A0D0,C2C1C0F0C0E2C0E0B0C0A0E0A1,A2A2C2F0F0D2C0A0E0C0D2C0C0E0B1B2,A2A0F0D0F0B0C0A0B0C0C1F2E1A1,C1F0A1E0C0B0B0C0E0D2C2,B2A0E0F0C2B0E2,F0B1D1F0||00";
	...
	*/
	...
	ReelSetController.prototype.symbolHighLightOnUpdate = function ({ caller: keyFrameAnim, time: timeDelta }) {
		...
		for (const item of groupData) {
			...
			//symbol.scalePlanetDisplay(scale);
		}
	};
	ReelSetController.prototype.symbolHighLightOnComplete = function () {
		...
		for (const item of groupData) {
			...
			const reel = this.getReelById(reelId);
			const symbol = reel.getSymbolById(symbolId);
			//symbol.scalePlanetDisplay(1);
			symbol.rotatePlanet(0);
			symbol.scalePlanet(1.2);
			gr.getTimer().setTimeout(
				function(){
					symbol.scalePlanet(1);
				}, config.timers.baseGame_scaleStayDuration);				
			symbol.hideHoneyComb();
		}
		const _this = this;
		gr.getTimer().setTimeout(
			function(){
				_this.presentSingleWinline(groupData);
			}, config.timers.baseGame_scaleStayDuration+30);
	...
	ReelSetController.prototype.presentSingleWinline = function (groupData) {
		...
		this.playSoundByConfig("SymbolCombine");
		for (const item of groupData) {
			const [reelId, symbolId] = this.translatePostionCode(item);
			const reel = this.getReelById(reelId);
			const reelOrderNum = reel.getReelOrderNum();
	...
	ReelSetController.prototype.goNext = function(){
		const _this = this;
		if (this.remainWinSequenceArray.length !== 0) {
	...
	ReelSetController.prototype.doReactDrop = function () {
		if (this.isEveryThingOkay()) {
			if (this.spaceshipCommander.isPlaying() === false && this.meteoriteCommander.isPlaying() === false){
				...
				const _this = this;
				gr.getTimer().setTimeout(function(){_this.doReactDrop();},500);
			}
		}
	};
		...
		if (this.remainWinSequenceArray.length === 1) {
			...
			switch (sequenceId) {
				case '38': // Standard Bonus is triggered
					gr.lib._spaceshipMain.spine.state.setAnimationByName(0, "bonus1_Win_Intro", true);
					// audio.play("Bonus1Triggered", 8);
					this.showNoMoreChainMsg('WBTriggered');
					break;
				case '39': // Action Bonus is triggered; 
					gr.lib._meteoriteMain.spine.state.setAnimationByName(0, "bonus2_Win_Intro", true);
					// audio.play("Bonus2Triggered", 8);
	...
	ReelSetController.prototype.showNoMoreChainMsg = function (msgType) {
		...
		if (this.needToShowNMC()) {
			...
			this.noMoreChainAnimation.bonusType = msgType;
			this.bonusTriggeredPanel.show(true);
			this.noMoreChainAnimation.play();
      this.playSoundByConfig("showBonusMsg");
		}
	};
	...
	ReelSetController.prototype.noMoreChainOnUpdate = function ({ caller: keyFrameAnim, time: timeDelta }) {
		...
		if (this.noMoreChainAnimation.status === 'fadeIn') {
			if(this.isPortrait()){
				scale = tweenFunc(timeDelta, 0, 1.2, duration);
			}else{
				scale = tweenFunc(timeDelta, 0, 1, duration);
			}
			...
			if(this.isPortrait()){
				scale = TweenFunctions.linear(timeDelta, 1.2, 0, duration);
			}else{
				scale = TweenFunctions.linear(timeDelta, 1, 0, duration);
			}
			this.bonusTriggeredPanel.updateCurrentStyle({ "_transform": { "_scale": { "_x": scale, "_y": scale } } });
		}
	};
	ReelSetController.prototype.noMoreChainOnComplete = function () {
		if (this.noMoreChainAnimation.status === 'fadeIn') {
			...
			this.bonusTriggeredPanel.show(false);
			this.sBonusFadeOutKeyAnimComplete();
		}
	};
	...
	ReelSetController.prototype.standardBonusFinished = function () {
		//it was unsubscribed because base game shall not monitoring this event during standard Bonus(warp bonus)
		msgBus.subscribe(LittleGreenMenGameEvent.eventIDs.SINGLE_SYMBOL_WIN_EXPLOSION_COMPLETE, this.symbolAnimationCompleteCallback);
		msgBus.subscribe(LittleGreenMenGameEvent.eventIDs.PAYTABLE_TEXT_ROLL_UP_COMPLETE, this.textRollingCompleteCallback);
		const _this = this;
		gr.getTimer().setTimeout(function () {
	...
	// 	//unsubscribed it because base game shall not monitoring this event during standard Bonus(warp bonus)
	// 	msgBus.unsubscribe(LittleGreenMenGameEvent.eventIDs.SINGLE_SYMBOL_WIN_EXPLOSION_COMPLETE, this.symbolAnimationCompleteCallback);
	// 	msgBus.unsubscribe(LittleGreenMenGameEvent.eventIDs.PAYTABLE_TEXT_ROLL_UP_COMPLETE, this.textRollingCompleteCallback);
	...
	ReelSetController.prototype.sBonusFadeOutKeyAnimComplete = function () {
		if (this.isEveryThingOkay()) {
			...
			if(this.noMoreChainAnimation.bonusType === 'WBTriggered'){
				...
				this.meteoriteCommander.reduceAwardNumberByOne();
				msgBus.publish(LittleGreenMenGameEvent.eventIDs.ACTION_BONUS_ENTRE_TRANSITION_START, {type:this.activeAction.type});
			}
			}else{
				const _this = this;
				gr.getTimer().setTimeout(function(){_this.sBonusFadeOutKeyAnimComplete();}, 500);
			}
			// if (this.sBonusTransitionKeyAnim.isPingPong) {
	...
	ReelSetController.prototype.exitActionBonus = function () {
		...
		if (this.activeAction.type == "S") {
			...
			// this.outterLightFadeKeyAnim.play();
			const _this = this;
			gr.getTimer().setTimeout(function () { _this.showWinSequence(); }, config.timers.action_spinActionShowWinSequence_delay);
	...
	*/
	ReelSetController.prototype.addSpriteAnimation = function () {
		const fullMovie_remove = this.findOrgSpriteSheetsByName('remove');
		...
		const fullMovie_spaceship = this.findOrgSpriteSheetsByName('bonus1');
		...
		const fullMovie_meteorite = this.findOrgSpriteSheetsByName('bonus2');
		...
		const fullMovie_et = this.findOrgSpriteSheetsByName('actionLittle');
		part1 = this.genNewSpriteArray(fullMovie_et, ["3~16", "17~32"]); // firing
		Sprite.addSpriteSheetAnimation('etFireBalls', part1);

		const fullMovie_trailing = this.findOrgSpriteSheetsByName('ActionTrailingLight');
		...
		for (let i = 1; i < 7; i++) {
			const fullMovie = this.findOrgSpriteSheetsByName('x0' + i + 'x');
		...
		const fullMovie_actionB = this.findOrgSpriteSheetsByName('actionB');
		...
		newArrayIndices.map(index => {
			if (typeof index === "number") {
				if (typeof sourceArray[index] !== "undefined") {
					...
					// Needs to change it back after animaiton is ready!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
					rtArr.push(sourceArray[0]);
	...
	/**
	 * This function is amd to avoid the undefined spritesheets when doing this reskin game.
	*/
	ReelSetController.prototype.findOrgSpriteSheetsByName = function (spriteSheetsName) {
		let rt = Sprite.getSpriteSheetAnimationFrameArray(spriteSheetsName);
		if (rt === undefined) {
			rt = [spriteSheetsName];
		}
		return rt;
